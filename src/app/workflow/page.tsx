...
export default function WorkflowPage() {
  ...
  }, [project, steps, toggleChecklistItem]);
  ...
  const getStatusBadge = (step: WorkflowStep) => {
    switch (step.status) {
      ...
      case 'skipped':
        return <Badge variant="secondary" className="bg-gray-400 text-white">건너뜀</Badge>;
      case 'in-progress':
  ...
  const handleSkipStep = (stepId: WorkflowStepId) => {
    skipStep(stepId);
  };
                ...
                {steps.map((step, index) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {step.status !== 'locked' && step.status !== 'skipped' && (
                        <ChevronRight className="h-5 w-5 text-muted-foreground" />
                      )}
                    </button>
                    {/* 건너뛰기 버튼 - available 또는 in-progress 상태일 때만 표시 */}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-6">
                      ...
                      <div className="flex items-center gap-3 pt-4 border-t">
                        ...
                        {/* 건너뛰기 버튼 */}
                        {(currentStepData.status === 'available' || currentStepData.status === 'in-progress') && (
                          <Button
                            variant="outline"
                        ...
                        {getNextAvailableStep() && currentStepData.status !== 'available' && currentStepData.status !== 'in-progress' && (
                          <Button
                            variant="outline"
                            onClick={() => {
